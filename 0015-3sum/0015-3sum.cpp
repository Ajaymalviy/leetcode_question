...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        sort(nums.begin() , nums.end());    //it is Sorted Array
        if(nums.size() < 3){    //if the size of array/vector is less than 3 then we directly return empty array
            return {};
        }
        ...
        for(int i = 0 ; i < nums.size() ; ++i){     //Iterate through the array to fix the first number of the triplet.
            ...
            int low = i + 1 , high = nums.size() - 1;   //Make two point high and low, and initialize sum as 0.
            int sum = 0;
